...
class LLCMData(data.Dataset):
    def __init__(self, data_dir, trial, transform=None, colorIndex = None, thermalIndex = None):
        ...
        # color_img_file -> 16946,  thermal_img_file -> 13975
        color_img_file, train_color_label = load_data(train_color_list)
        thermal_img_file, train_thermal_label = load_data(train_thermal_list)
        ...
        self.cIndex = colorIndex  # 需要每个Epoch更新一次
        self.tIndex = thermalIndex

    def __getitem__(self, index):
        # index为sampler生成
        # 任意index，都返回同label img

        img1 = self.train_color_image[self.cIndex[index]]
...
def load_data(input_data_path ):
    with open(input_data_path) as f:
        ...
        # file_cam =  [int(s.split(' ')[1]) for s in data_file_list]
